...
#define GWIP_2               4                 // IP is no part of our subnet)
#define GWIP_3               2
#define GWIP_4               1
...
unsigned int ISNGenHigh;                         // upper word of our Initial Sequence Number
...
unsigned int RecdFrameLength;                    // CS8900 reported frame length
unsigned int RecdFrameMAC[3];                    // 48 bit MAC
unsigned int RecdFrameIP[2];                     // 32 bit IP
unsigned int RecdIPFrameLength;                  // 16 bit IP packet length

// the next 3 buffers must be word-aligned!
// (here the 'RecdIPFrameLength' above does that)
unsigned char TxFrame1[ETH_HEADER_SIZE + IP_HEADER_SIZE + TCP_HEADER_SIZE + MAX_TCP_TX_DATA_SIZE];
unsigned char TxFrame2[ETH_HEADER_SIZE + MAX_ETH_TX_DATA_SIZE];
unsigned char RxTCPBuffer[MAX_TCP_RX_DATA_SIZE]; // space for incoming TCP-data

unsigned int TxFrame1Size;                       // bytes to send in TxFrame1
...
interrupt (TIMERA1_VECTOR) TCPClockHandler(void);// interrupt function, called every 262 ms
...
extern void TCPLowLevelInit(void);                      // setup timer, LAN-controller, flags...
